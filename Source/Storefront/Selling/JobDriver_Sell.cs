   ...
   /* public class JobDriver_Sell : JobDriver
    {
        ...
        public override bool TryMakePreToilReservations(bool errorOnFailed)
        {
            ...
            if (diningSpot == null)
            {
                Log.Warning($"{pawn.NameShortColored} FAILED to sell {patron?.NameShortColored}, because no dining spot is set. patronJob = {patron?.jobs.curDriver?.GetType().Name}");
                return false;
            }

            if (food == null)
            {
                Log.Warning($"{pawn.NameShortColored} FAILED to sell {patron.NameShortColored}, because food is not set.");
                return false;
            }

            if (food.ParentHolder != food.Map)
            {
                Log.Warning($"{pawn.NameShortColored} FAILED to sell {food.Label} to {patron.NameShortColored}, because it is inside {(food.ParentHolder is Thing parentThing ? parentThing.Label : food.ParentHolder.ToString())}");
                return false;
            }

            if (!pawn.Resell(food, job, food.stackCount, 1, null, errorOnFailed))
            {
                Log.Message($"{pawn.NameShortColored} FAILED to resell food {food.Label}.");
                return false;
            }
            ...
            //Log.Message($"{pawn.NameShortColored} reselld food {food.Label}.");
            job.count = 1;
