...
{
    public class StoreStock : IExposable
    {
        public class Stock
        {
            public ThingDef def;
        ...
        //private readonly List<Thing> stockCache = new List<Thing>();
        private readonly List<ConsumeOptimality> eatOptimalityCache = new List<ConsumeOptimality>();
        private readonly List<ConsumeOptimality> joyOptimalityCache = new List<ConsumeOptimality>();
        private Map Map => Store.Map;
        private StoreController Store { get; }
        private readonly Dictionary<ThingDef, Stock> stockCache = new Dictionary<ThingDef, Stock>();
        public IReadOnlyDictionary<ThingDef, Stock> AllStock => stockCache;

        public readonly List<Thing> items = new List<Thing>();

        public StoreStock(StoreController store)
        {
            Store = store;
        }
        ...
        public bool HasAnyFoodFor(Pawn pawn, bool allowDrug)
        {
        ...
        public class FoodOptimality
        {
            ...
            public StoreController Store { get; }

            public FoodOptimality(StoreController store, Thing thing, float optimality)
            {
                Thing = thing;
        ...
        public static Thing GetBestMealFor(IEnumerable<StoreController> stores, Pawn pawn, out StoreController store, bool allowDrug, bool includeEat = true, bool includeJoy = true)
        {
            store = null;
            if (stores == null) return null;
            var options = stores.SelectMany(controller => controller.Stock.GetMealOptions(pawn, allowDrug, includeEat, includeJoy));
            ...
            {
                store = best.Store;
                //Log.Message($"{pawn.NameShortColored}: GetBestMealFor: {best.Thing.LabelCap} with optimality {best.Optimality:F2} at {store?.Name}.");
                return best.Thing;
            }
            return null;
        }

        public static Thing GetRandomMealFor(IEnumerable<StoreController> stores, Pawn pawn, out StoreController store, bool allowDrug, bool includeEat = true, bool includeJoy = true)
        {
            store = null;
            if (stores == null) return null;
            var options = stores.SelectMany(controller => controller.Stock.GetMealOptions(pawn, allowDrug, includeEat, includeJoy));
            ...
            {
                store = random.Store;
                //Log.Message($"{pawn.NameShortColored} picked {random.Thing.Label} with a score of {random.Optimality} at {store?.Name}.\nOptions were:\n{options.Select(o=>$"- {o.Thing.LabelCap} ({o.Optimality:F0}) at {o.Store.Name}").ToLineList()}");
                return random.Thing;
            }
            return null;
        }

        private IEnumerable<FoodOptimality> GetMealOptions(Pawn pawn, bool allowDrug, bool includeEat, bool includeJoy)
        {
            return stockCache.Values
                ...
                //.Where(consumable => Store.Orders.CanBeOrdered(consumable))
                .Select(consumable => new FoodOptimality(Store, consumable, GetMealOptimalityScore(pawn, consumable, includeEat, includeJoy)))
                .Where(def => def.Optimality >= MinOptimality);
        }

        private bool CanAfford(Pawn pawn, ThingDef def)
        {
            if (Store.guestPricePercentage <= 0) return true;
            //if (!pawn.CanHaveDebt()) return true;
            return pawn.GetSilver() >= def.GetPrice(Store);
        }

        private float GetMealOptimalityScore(Pawn pawn, Thing thing, bool includeEat = true, bool includeJoy = true)
        {
        ...
        private static float CalcEatOptimality(Pawn pawn, Thing thing)
        {
            return Mathf.Max(0, FoodUtility.FoodOptimality(pawn, thing, thing.def, 25));
        }

        private static float CalcJoyOptimality(Pawn pawn, Thing thing)
        {
            var def = thing.def;
        ...
        private static float GetCachedOptimality(Pawn pawn, Thing thing, List<ConsumeOptimality> optimalityCache, Func<Pawn, Thing, float> calcFunction)
        {
       ...
       /* public Thing GetServableThing(Order order, Pawn pawn)
        ...
        private void FindStockItems()
        {
            ...
            Log.Message($"items: {items}");
            /*
            foreach (var thing in GetConsumablesOnMap()
                .Where(t => t.def is { IsIngestible: true, IsCorpse: false } && Menu.IsOnMenu(t) && !t.IsForbidden(Faction.OfPlayer) && Store.GetIsInRange(t.Position)))
            {
        ...
        public bool IsAvailable(Thing consumable)
        {
